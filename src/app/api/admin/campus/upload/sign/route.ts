...
const ALLOWED_FOLDERS = new Set([
    'campus/photo-strip',
    'campus/facilities/learning',
    'campus/facilities/outdoor',
...
export async function POST(req: NextRequest) {
    ...
    const { folder, filename, contentType, fileSizeMb } = body as Record<string, unknown>
    ...
    const storagePath = `${folder}/${Date.now()}-${safeName.replace(/\.[^.]+$/, '')}${ext}`
